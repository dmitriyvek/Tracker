...
def get_arg_parser(params: dict) -> argparse.ArgumentParser:
    '''
    Configure and return parser with given default
    params for command line variables
    '''
    from tracker.utils.settings import ENV_PATH

    parser = argparse.ArgumentParser(
        allow_abbrev=False,
        description=f'The default values specified here are taken \
            from .env file located here: {ENV_PATH} \
            (if you want to specify another location change ENV_PATH \
            in tracker.utils.settings)',
    ...
    group.add_argument(
        '--pg-pool-min-size',
        type=positive_int,
        default=params['pg_pool_min_size'],
        required=False, help='Minimum database connections'
    )
    group.add_argument(
        '--pg-pool-max-size',
        type=positive_int,
        default=params['pg_pool_max_size'],
        required=False,
